#!/bin/bash
# deploy/sync_to_srv.sh — **repo 있는 머신에서** 최초 부트스트랩용(git-free 전환). host/* → /srv/fcmanager.
#
# 상시 배포는 git-free 다: /srv/fcmanager/deploy-prod.sh X.Y.Z 가 이미지에서 모든 host 파일을
# 추출하고, 부트스트랩 파일(deploy-prod.sh · _extract_and_deploy.sh)까지 self-heal 하므로
# 이후 repo/git pull 은 영영 불필요.
#
# repo 가 없는 운영 호스트는 이 스크립트 대신 **이미지에서 직접**(git-free) 부트스트랩할 수 있다:
#   cd /srv/fcmanager && CID=$(docker create honestjung/fcmanager:X.Y.Z)
#   for f in _extract_and_deploy.sh deploy-prod.sh; do docker cp "$CID:/app/deploy/host/$f" ./; done
#   docker rm "$CID" && chmod +x _extract_and_deploy.sh deploy-prod.sh
#
# 최초 1회: /srv/fcmanager 생성 + .env 준비 후 실행.
set -euo pipefail
